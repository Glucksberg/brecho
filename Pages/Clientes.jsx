...
export default function Clientes() {
  ...
  const getClientStats = (clienteId) => {
    const clientSales = vendas.filter(v => v.cliente_id === clienteId);
    const totalSpent = clientSales.reduce((sum, v) => sum + v.valor_venda, 0);
    return {
  ...
  const handleSubmit = async (clientData) => {
    try {
      if (editingCliente) {
        await Cliente.update(editingCliente.id, clientData);
      } else {
          ...
          <AnimatePresence>
            {filteredClientes.map((cliente) => (
              <ClienteCard
                key={cliente.id}
                cliente={cliente}
                stats={getClientStats(cliente.id)}
                onEdit={handleEdit}
